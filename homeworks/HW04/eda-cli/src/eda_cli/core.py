...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Новая эвристика 1: слишком много одинаковых значений (риск константы)
    max_duplicate_share = 0.0
    for col in summary.columns:
        if col.non_null > 0:
            duplicate_share = 1.0 - (col.unique / col.non_null)
            max_duplicate_share = max(max_duplicate_share, duplicate_share)
    flags["max_duplicate_share"] = max_duplicate_share
    flags["too_many_duplicates"] = max_duplicate_share > 0.9  # >90% одинаковых значений
    
    # Новая эвристика 2: слишком высокая кардинальность (риск ID-поля)
    max_cardinality_ratio = 0.0
    for col in summary.columns:
        if col.non_null > 0:
            cardinality_ratio = col.unique / col.non_null
            max_cardinality_ratio = max(max_cardinality_ratio, cardinality_ratio)
    flags["max_cardinality_ratio"] = max_cardinality_ratio
    flags["too_high_cardinality"] = max_cardinality_ratio > 0.95  # уникальных почти столько же сколько строк
         
    
    # Простейший «скор» качества
    score = 1.0
    score -= max_missing_share  # чем больше пропусков, тем хуже
    score -= max_duplicate_share * 0.5  # дубликаты тоже плохо
    score -= (max_cardinality_ratio - 0.5) * 0.3 if max_cardinality_ratio > 0.5 else 0  # ID-поля не очень полезны
