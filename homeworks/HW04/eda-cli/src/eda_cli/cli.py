...
def report(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    max_hist_columns: int = typer.Argument(..., help="Максимум числовых колонок для гистограмм."),
    title: str = typer.Argument(..., help="Заголовок отчета")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        f.write(f"# EDA-отчёт\n\n")
        f.write(f"Заголовок отчета {title} \n\n")
        ...
        f.write(f"- Риск константы: **{quality_flags['max_duplicate_share']}**\n")
        f.write(f"- Риск ID-поля: **{quality_flags['max_cardinality_ratio']}**\n\n")
